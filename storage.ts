...
export interface IStorage {
  ...
  // Strategy methods
  getMonsterStrategies(monsterId: number): Promise<Strategy[]>;
  ...
  getMonsterInsights(monsterId: number): Promise<MonsterInsight[]>;
  createMonsterInsight(insight: InsertMonsterInsight): Promise<MonsterInsight>;
...
export class MemStorage implements IStorage {
  private monstersData: Map<number, Monster>;
  private strategiesData: Map<number, Strategy>;
  private questionsData: Map<number, Question>;
  private userAnswersData: Map<number, UserAnswer>;
  private monsterInsightsData: Map<number, MonsterInsight>;
  
  private monstersCurrentId: number;
  private strategiesCurrentId: number;
  private questionsCurrentId: number;
  private userAnswersCurrentId: number;
  private insightsCurrentId: number;
  ...
  async createMonster(insertMonster: InsertMonster): Promise<Monster> {
    ...
    const monster: Monster = { ...insertMonster, id };
    this.monstersData.set(id, monster);
    return monster;
  }

  // Strategy methods
  async getMonsterStrategies(monsterId: number): Promise<Strategy[]> {
  ...
  // Monster insight methods
  async getMonsterInsights(monsterId: number): Promise<MonsterInsight[]> {
    return Array.from(this.monsterInsightsData.values())
      .filter((insight) => insight.monsterId === monsterId)
      .sort((a, b) => a.order - b.order);
  }

  async createMonsterInsight(insertInsight: InsertMonsterInsight): Promise<MonsterInsight> {
    const id = this.insightsCurrentId++;
    const insight: MonsterInsight = { ...insertInsight, id };
