...
export function calculateResult(
  ...
  const scoreMap: Record<string, number> = { A: 0, B: 0, C: 0, D: 0, E: 0, F: 0 };
  ...
  if (!finalType) throw new Error("점수 계산 실패");

  const matchedMonster = monsters.find((m) => m.types.includes(finalType));
  if (!matchedMonster) throw new Error(`해당 유형 몬스터 없음: ${finalType}`);
